...
FrameInfo *generate_frame_info(callsite_header_t *callsite, function_info_t *fn) {
    ...
    */
    ...
    FrameInfo *frame = static_cast<FrameInfo *>(malloc(size_of_frame(numSlots)));
...
StatepointTable *StatepointTable::generateTable(void *map, float load_factor) {
...
size_t FrameInfo::size() {
    return size_of_frame(numSlots);
}

// returns the next frame relative the current frame
FrameInfo *next_frame(FrameInfo *cur) {
    uint8_t *next = ((uint8_t *) cur) + cur->size();
...
StatepointTable *new_table(float loadFactor, uint64_t expectedElms) {
    ...
    TableBucket *buckets = static_cast<TableBucket*>(calloc(numBuckets, sizeof(TableBucket)));
...
// function).
void StatepointTable::insertKey(uint64_t key, FrameInfo *value) {
    uint64_t idx = computeBucketIndex(this, key);
    TableBucket *bucket = buckets + idx;

    if (bucket->numEntries == 0) {
        bucket->numEntries = 1;
        bucket->sizeOfEntries = value->size();
        bucket->entries = value;
    } else {
        // a collision occured!
        size_t newSize = bucket->sizeOfEntries + value->size();
        FrameInfo *newEntries = static_cast<FrameInfo *>(realloc(bucket->entries, newSize));

        assert(newEntries && "bad alloc");

        // copy value onto the end of the possibly resized entry array
        FrameInfo *oldEnd = (FrameInfo *) (
                ((uint8_t *) newEntries) + bucket->sizeOfEntries
        );

        memmove(oldEnd, value, value->size());
...
FrameInfo *StatepointTable::lookupReturnAddress(uint64_t retAddr) {
    uint64_t idx = computeBucketIndex(this, retAddr);
    TableBucket bucket = buckets[idx];
...
void StatepointTable::print(FILE *stream, bool skip_empty) {
    for (uint64_t i = 0; i < size; i++) {
...
void FrameInfo::print(FILE *stream) {
